...
- [ ] T006 [UI] Create `NavItem` component: renders a single MUI `ListItemButton` with an icon and an optional label; accepts `icon`, `label`, `href`, `active`, `collapsed`, `tooltip` props; uses Next.js `Link`; shows MUI `Tooltip` on hover when `collapsed` is true — `source/Testurio.Web/src/components/NavItem/NavItem.tsx`
...
- [ ] T008 [UI] Create `PrivateCabinetLayout` component: assembles `AppHeader` and `AppSidebar` into the full shell; renders `children` in the main content area; uses MUI `Box` for the flex layout — `source/Testurio.Web/src/components/PrivateCabinetLayout/PrivateCabinetLayout.tsx`
