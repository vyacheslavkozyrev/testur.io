...
- [x] T005 [Infra] Ensure `TestResultRepository` round-trips the `steps` nested array when writing/reading `ScenarioSummary` entries in Cosmos — `source/Testurio.Infrastructure/Cosmos/TestResultRepository.cs`
- [x] T006 [UI] Extend `ScenarioSummary` TypeScript type with `steps: StepSummary[] | null` and add `StepSummary` interface (`stepIndex: number`, `action: string`, `passed: boolean`, `errorMessage: string | null`, `screenshotBlobUri: string | null`) — `source/Testurio.Web/src/types/history.types.ts`
- [x] T007 [UI] Update MSW mock handler for `GET /v1/stats/projects/:projectId/runs/:runId` to return `steps` arrays on `ui_e2e` scenario entries and `null` on `api` entries — `source/Testurio.Web/src/mocks/handlers/history.ts`
